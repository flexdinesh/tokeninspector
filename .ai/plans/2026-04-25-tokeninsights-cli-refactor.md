# Plan: Maintainable Go+SQLite patterns for tokeninsights-cli

## Decisions

- **Package split**: proper (`internal/db` + `internal/cli` + `cmd/tokeninsights-cli`)
...
- Add `PRAGMA user_version = 1` after `CREATE TABLE IF NOT EXISTS` in `plugins/oc-tokeninsights-writer.ts`
- Add `PRAGMA user_version = 1` after `CREATE TABLE IF NOT EXISTS` in `plugins/oc-tokeninsights-server.ts`
...
### Task F: `cmd/tokeninsights-cli/main.go`
...
- `cmd/tokeninsights-cli/main_test.go`: end-to-end `run()` with seeded temp DB
